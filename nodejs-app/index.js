...
const esClient = new Client({ node: 'http://elasticsearch:9200', auth: {
    username: 'admin',
    password: 'password',
  },}, );
...
const Document = mongoose.model('Document', {
  ...
  const createAndSaveDocument = async () => {
    ...
    try {
      ...
      console.log(`Document indexed successfully in Elasticsearch. Document ID: ${response.body._id}`);


      return {mongo: savedDocument._id, elastic: response.body._id}
    } catch (err) {
